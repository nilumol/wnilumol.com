...
Opportunities also has a passphrase-gated manual URL intake (`JobAgentOpportunityIntake.tsx`,
`POST /api/job-agent/opportunities`) for canonical Greenhouse/Lever/Ashby posting URLs. It uses
the existing ledger-entry identity/row model and persists the entry plus captured posting content
in the private Blob JSON document `job-agent/manual-opportunities.json`; see
`scripts/job-agent/{manual-opportunity-extract,manual-opportunities,opportunity-context}.ts` and
the "Manual URL intake" section of `docs/job-agent.md`. Tailor, scan/draft, and status routes
resolve both the git ledger and this manual store.
