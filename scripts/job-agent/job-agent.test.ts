...
test("matchJobAgentKeywordFamily matches the five target families case-insensitively", () => {
  assert.equal(matchJobAgentKeywordFamily("Senior solutions consultant"), "Solutions Consultant");
  assert.equal(matchJobAgentKeywordFamily("SOLUTIONS ARCHITECT II"), "Solutions Architect");
  assert.equal(matchJobAgentKeywordFamily("Customer Success Manager, EMEA"), "Customer Success Manager");
  assert.equal(matchJobAgentKeywordFamily("Sales Engineer - Public Sector"), "Sales Engineer");
});

test("matchJobAgentKeywordFamily matches Implementation Manager abbreviation variants", () => {
  for (const title of [
    ...
    assert.equal(matchJobAgentKeywordFamily(title), "Implementation Manager", `expected match for "${title}"`);
  }
});

test("matchJobAgentKeywordFamily returns null for unrelated titles", () => {
  assert.equal(matchJobAgentKeywordFamily("Software Engineer II"), null);
  assert.equal(matchJobAgentKeywordFamily("Staff Accountant"), null);
});
...
test("fetchLeverBoardJobs normalizes a posting with a structured salaryRange", async () => {
  const board: JobAgentBoard = { token: "ethena", label: "Ethena", source: "lever" };
  const fixture = [
    {
...
test("fetchLeverBoardJobs falls back to null structuredCompensationRange when salaryRange is absent", async () => {
  const board: JobAgentBoard = { token: "veeva", label: "Veeva", source: "lever" };
  const fixture = [
    {
      id: "abc123",
...
test("fetchLeverBoardJobs treats a 404 as board-not-found rather than throwing", async () => {
  const board: JobAgentBoard = { token: "unknown-co", label: "Unknown Co", source: "lever" };
...
test("fetchAshbyBoardJobs treats a 404 as board-not-found rather than throwing", async () => {
  const board: JobAgentBoard = { token: "unknown-co", label: "Unknown Co", source: "ashby" };
...
test("ledger save/load round-trips and always writes sorted by key, numeric-aware within a source", () => {
  const dir = mkdtempSync(join(tmpdir(), "job-agent-ledger-"));
  ...
  const keysInFile = Object.keys(JSON.parse(raw) as JobAgentLedger);
...
test("loadLedger returns an empty object when the file doesn't exist", () => {
  const missingPath = join(mkdtempSync(join(tmpdir(), "job-agent-missing-")), "does-not-exist.json");
...
test("runJobAgentPipeline: filters, skips already-seen, adds new candidates as pending, expires missing ones", async () => {
  const acme: JobAgentBoard = { token: "acme", label: "Acme", source: "greenhouse" };
  const ghost: JobAgentBoard = { token: "ghost", label: "Ghost Co", source: "greenhouse" };
  ...
  const result = await runJobAgentPipeline([acme, ghost], ledger, {
...
test("runJobAgentPipeline: routes Lever/Ashby boards to their own fetchers, uses structured compensation when available, and stamps postedAt", async () => {
  const ethena: JobAgentBoard = { token: "ethena", label: "Ethena", source: "lever" };
  const notion: JobAgentBoard = { token: "notion", label: "Notion", source: "ashby" };

  const fetchGreenhouse = async (): Promise<never> => {
    throw new Error("greenhouse fetcher should not be called for lever/ashby boards");
  };
  const fetchLever = async (board: JobAgentBoard) => ({
    board,
    found: true,
    jobs: [
      {
  ...
  const fetchAshby = async (board: JobAgentBoard) => ({
    board,
    found: true,
    jobs: [
      {
  ...
  const result = await runJobAgentPipeline([ethena, notion], ledger, {
    fetchers: { greenhouse: fetchGreenhouse, lever: fetchLever, ashby: fetchAshby },
    log: () => {},
  });
...
test("runJobAgentPipeline: maps Greenhouse's first_published into the ledger's postedAt field", async () => {
  const acme: JobAgentBoard = { token: "acme", label: "Acme", source: "greenhouse" };
