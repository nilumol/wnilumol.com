...
const SCORING_RUBRIC_PATH = join(REPO_ROOT, "content/job-agent-scoring.md");
 ...
 * `npm run job-agent:apply-scores` flow, which remains the default way scores get recorded.
 */
async function main(): Promise<void> {
  ...
  let scoredCount = 0;
  try {
    for (const board of jobAgentBoards) {
      let fetchResult;
      try {
        ...
        console.log(
          `job-agent:score-via-api: failed to fetch board "${board.token}" (${board.label}): ${(error as Error).message}`,
        );
        continue;
      }
      ...
      for (const candidate of candidates) {
        ...
        console.log(`job-agent:score-via-api: scoring "${candidate.job.title}" (${candidate.company})`);
  ...
  console.log(`job-agent:score-via-api: scored ${scoredCount} pending job(s).`);
}
