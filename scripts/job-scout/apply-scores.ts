...
export function parseScoreBatch(raw: string): ScoreBatchEntry[] {
  ...
  try {
    ...
    throw new Error(`job-scout: apply-scores input is not valid JSON: ${(error as Error).message}`);
  }
  if (!Array.isArray(parsed)) {
    throw new Error("job-scout: apply-scores input must be a JSON array.");
  }
  parsed.forEach((entry, index) => {
    if (!isScoreBatchEntry(entry)) {
      throw new Error(
        `job-scout: apply-scores input entry at index ${index} is malformed - expected ` +
 ...
 * the ledger.
 */
export function applyScoresToLedger(ledger: JobScoutLedger, batch: ScoreBatchEntry[]): void {
  for (const entry of batch) {
    const existing = ledger[entry.id];
    if (!existing) {
      throw new Error(`job-scout: apply-scores: no ledger entry found for id "${entry.id}".`);
    }
    if (existing.status !== "pending") {
      throw new Error(
        `job-scout: apply-scores: ledger entry "${entry.id}" is not "pending" (status is ` +
...
function main(): void {
  ...
  if (!inputPath) {
    throw new Error("usage: npm run job-scout:apply-scores -- <path-to-json>");
  }
  ...
  console.log(`job-scout: applied ${batch.length} score(s) to the ledger.`);
}

// Only runs the CLI when this file is executed directly (`npm run job-scout:apply-scores`),
