...
import type { CandidateJob, GreenhouseJob } from "./types.ts";
 ...
 */
export function filterCandidateJobs(board: JobScoutBoard, jobs: GreenhouseJob[]): CandidateJob[] {
  const candidates: CandidateJob[] = [];
  for (const job of jobs) {
    const keywordFamily = matchJobScoutKeywordFamily(job.title);
    if (keywordFamily) {
      candidates.push({ job, company: board.label, keywordFamily });
    }
  }
  return candidates;
}
